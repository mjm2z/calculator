...
describe('Button Tests', () => {
    ...
    it('Given the calculator, when any number is clicked, then it should be appended to the current equation', ()=>{
        ...
        btAC.simulate('click'); 
        expect(store.getState()).toEqual({equation: {currentEquation: '0', displayValue: '0'}});
        btOne.simulate('click');   
        expect(store.getState()).toEqual({equation: {currentEquation: '01', displayValue: '1'}});
        btTwo.simulate('click');    
        expect(store.getState()).toEqual({equation: {currentEquation: '02', displayValue: '2'}});
        btThree.simulate('click');    
        expect(store.getState()).toEqual({equation: {currentEquation: '03', displayValue: '3'}});
        btFour.simulate('click');    
        expect(store.getState()).toEqual({equation: {currentEquation: '04', displayValue: '4'}});
        btFive.simulate('click');    
        expect(store.getState()).toEqual({equation: {currentEquation: '05', displayValue: '5'}});
        btSix.simulate('click');    
        expect(store.getState()).toEqual({equation: {currentEquation: '06', displayValue: '6'}});
        btSeven.simulate('click');    
        expect(store.getState()).toEqual({equation: {currentEquation: '07', displayValue: '7'}});
        btEight.simulate('click');    
        expect(store.getState()).toEqual({equation: {currentEquation: '08', displayValue: '8'}});
        btNine.simulate('click');    
        expect(store.getState()).toEqual({equation: {currentEquation: '09', displayValue: '9'}});
        btZero.simulate('click');    
        expect(store.getState()).toEqual({equation: {currentEquation: '00', displayValue: '0'}});
    })
    ...
    it('Given the calculator, when the positive/negative button is clicked, then the sign of the current number should be switched', ()=>{
        ...
        btPosNeg.simulate('click'); 
        expect(store.getState()).toEqual({equation: {currentEquation: '099*(-9', displayValue: '-9'}});

        store.dispatch({type: 'EQUATION', equation: '09+'})
        reset();
        btPosNeg = buttons().dive().dive().find('#bt1')
        btPosNeg.simulate('click'); 
        expect(store.getState()).toEqual({equation: {currentEquation: '09+(-0', displayValue: '-0'}});

        store.dispatch({type: 'EQUATION', equation: '5.123'})
        reset();
        btPosNeg = buttons().dive().dive().find('#bt1')
        btPosNeg.simulate('click'); 
        expect(store.getState()).toEqual({equation: {currentEquation: '(-5.123', displayValue: '-5.123'}});
        ...
        btPosNeg = buttons().dive().dive().find('#bt1')
        btPosNeg.simulate('click'); 
        expect(store.getState()).toEqual({equation: {currentEquation: '099*8.90123+(-0', displayValue: '-0'}});
    })
    ...
    it('Given the calculator, when the decimal button is clicked, then it should be appended to the current equation if applicable', ()=>{
        ...
        btDecimal.simulate('click'); 
        expect(store.getState()).toEqual({equation: {currentEquation: '0.18/6+(-1.', displayValue: '-1.'}});

        store.dispatch({type: 'EQUATION', equation: '0.18/6+'})
        reset();
        btDecimal = buttons().dive().dive().find('#bt17')
        btDecimal.simulate('click'); 
        expect(store.getState()).toEqual({equation: {currentEquation: '0.18/6+0.', displayValue: '0.'}});

        store.dispatch({type: 'EQUATION', equation: '0.5*15'})
        reset();
        btDecimal = buttons().dive().dive().find('#bt17')
        btDecimal.simulate('click'); 
        expect(store.getState()).toEqual({equation: {currentEquation: '0.5*15.', displayValue: '15.'}});

        store.dispatch({type: 'EQUATION', equation: '01*15.6'})
        reset();
        btDecimal = buttons().dive().dive().find('#bt17')
        btDecimal.simulate('click'); 
        expect(store.getState()).toEqual({equation: {currentEquation: '01*15.6', displayValue: '15.6'}});

        store.dispatch({type: 'EQUATION', equation: '01*15.'})
        reset();
        btDecimal = buttons().dive().dive().find('#bt17')
        btDecimal.simulate('click'); 
        expect(store.getState()).toEqual({equation: {currentEquation: '01*15.', displayValue: '15.'}});
    })
    ...
    it('Given the calculator, when the percent button is clicked, then the current number should be divided by 100', ()=>{
        ...
        expect(store.getState()).toEqual({equation: {currentEquation: '0.01', displayValue: '0.01'}});

        store.dispatch({type: 'EQUATION', equation: '01*15.6'})
        reset();
        btPercent = buttons().dive().dive().find('#bt2');
        btPercent.simulate('click'); 
        expect(store.getState()).toEqual({equation: {currentEquation: '01*0.156', displayValue: '0.156'}});

        store.dispatch({type: 'EQUATION', equation: '01*0.156'})
        reset();
        btPercent = buttons().dive().dive().find('#bt2');
        btPercent.simulate('click'); 
        expect(store.getState()).toEqual({equation: {currentEquation: '01*0.00156', displayValue: '0.00156'}});
    })
    ...
    it('Given the calculator, when an operation is clicked and there are no operators at the end of the equation, then the operator should be appeneded to the end of the equation', ()=>{
        ...
        expect(store.getState()).toEqual({equation: {currentEquation: '0123', displayValue: '123'}});
    ...
    it('Given the calculator, when the equation is complete and the equal button is clicked, the correct solution should display and replace the equation', () => {
        ...
        expect(store.getState()).toEqual({equation: {currentEquation: '123', displayValue: '123'}});

        store.dispatch({type: 'EQUATION', equation: '01234+'})
        reset();
        btEqual = buttons().dive().dive().find('#bt18');
        btEqual.simulate('click'); 
        expect(store.getState()).toEqual({equation: {currentEquation: '1234', displayValue: '1234'}});

        store.dispatch({type: 'EQUATION', equation: '01234-234'})
        reset();
        btEqual = buttons().dive().dive().find('#bt18');
        btEqual.simulate('click'); 
        expect(store.getState()).toEqual({equation: {currentEquation: '1000', displayValue: '1000'}});

        store.dispatch({type: 'EQUATION', equation: '01234+4321 + 0.003'})
        reset();
        btEqual = buttons().dive().dive().find('#bt18');
        btEqual.simulate('click'); 
        expect(store.getState()).toEqual({equation: {currentEquation: '5555.003', displayValue: '5555.003'}});

        store.dispatch({type: 'EQUATION', equation: '(-123)*0.123+123'})
        reset();
        btEqual = buttons().dive().dive().find('#bt18');
        btEqual.simulate('click'); 
        expect(store.getState()).toEqual({equation: {currentEquation: '107.871', displayValue: '107.871'}});

        store.dispatch({type: 'EQUATION', equation: '(-123)*0.123+'})
        reset();
        btEqual = buttons().dive().dive().find('#bt18');
        btEqual.simulate('click'); 
        expect(store.getState()).toEqual({equation: {currentEquation: '(-15.129', displayValue: '-15.129'}});
    })
});